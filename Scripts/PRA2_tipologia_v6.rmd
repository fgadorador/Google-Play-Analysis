...
Por último, transformaremos la variable *Last.Updated* a "YYYY-MM_DD" para que tenga un formato más legible y sea más fácil acceder al día, mes o año en caso de requerirlo.
...
Antes de factorizar en categorías las variables *chr*, primero analizamos el número de valores diferentes que tiene cada variable:
...
A partir de estos números, determinamos que será efectivo factorizar las variables *Category*, *Installs*, *Type*, *Content.rating*, *Android.Ver* y *Multi_Genre*, de esta forma, aplicando as.factor, nos aseguramos de que son variables categóricas:
...
## Creación de fichero preprocesado

```{r message= FALSE, warning=FALSE}

write.csv(data_clean, 'googleplay_clean.csv')
```
...
El test Kruskal indica que existen diferencias significativas entre al menos dos de las categorias comparadas, sin embargo, no proporciona información de que pares de categorías presentan medianas significativamente diferentes. Por este motivo, realizaremos la prueba de Mann-Whitney (empleando adicionalmente la correción de Bonferroni ya que estamos realizando múltiples comparaciones) con el objetivo de analizar en que pares de categorias independientes existen diferencias, es decir, si la mediana de una categoria es igual (hipotesis nula) o distinta (hipotesis alternativa) a la mediana de la otra (por pares). Un p-valor menor de 0.05 indicará que existe una diferencia significativa en el *mean rank* de las categorías. En el caso en que las varianzas sean parecidas entre categorías querrá decir que las medias son significativamente diferentes, pero tal y como se ha demostrado en el test Fligner-Killeen anterior, en general la varianza de las categorías no es homogenia, almenos difiere en una de las categorías.

```{r message= FALSE, warning=FALSE}
pairwise.wilcox.test(data_clean$Rating, data_clean$Category, p.adj = "bonf")
```

De la tabla anterior se puede concluir, por ejemplo, que las categorías *Dating*, *Health and fitness*, *Personalization* son unas de las que más diferencias significativas muestran al hacer el análisis a pares, concretamente 13, 8 y 11 diferencias respectivamente. Por otro lado, alguna de las características que se encuentran en un punto medio y no muestran discrepancias a pares son *Auto and vehicles*, *Beauty*, *Business*, *Libraries and demo* y *Parenting*.
...
### Creación de un modelo de regresión: Alternativas a random forest

Con el objetivo de encontrar un modelo que explique mejor el rating de las aplicaciones, vamos a estudiar el uso de modelos polinómicos, regresión lasso y support vector machines para ver si mejoran el poder de predicción.

En primer lugar mostramos el modelo lasso:
...
x <- data.matrix(train[, c('Category', "Reviews", "Size", 'Installs', 'Type', 'Price', 'Content.Rating', 'Android.Ver', 'Multi_Genre')])
...
En el mejor de los casos, este modelo muestra un error cuadrático medio de 0.265, ligeramente superior al modelo random forest.

A continuación analizamos la regresión polinómica:

```{r message= FALSE, warning=FALSE}
modelpolin <- lm(Rating ~ Category + Reviews + Size + Installs + Type + Price + Content.Rating + Android.Ver + Multi_Genre, data = train)
summary(modelpolin)
```

Este modelo muestra un error cuadrático de 0.25 aproximadente, ligeramente superior al random forest, y coeficiente R² de 0.08, lo que nos indica que la aproximación polinómica no es una buena alternativa.

Por último analizamos el modelo SVM:

```{r message= FALSE, warning=FALSE}
library(e1071)
model_svm <- svm(Rating ~ Category + Reviews + Size + Installs + Type + Price + Content.Rating + Android.Ver + Multi_Genre, data = train)
summary(model_svm)
```
...
residuals_svm <- test$Rating - pred_svm
RMSE_svm <- rmse(pred_svm, test$Rating)
print(RMSE_svm**2)
```

El modelo SVM presenta un error cuadrático de 0.29, por lo que ningún modelo ha podido predecir de una forma significativamente mejor al random forest.
...
La visualización mediante tablas y gráficas nos permitirá fácilmente comprender cómo las variables están distribuidas y que relaciones existentes observamos entre ellas. Además, las visualizaciones permitirán reafirmar y comprobar los resultados de los análisis estadísticos realizados previamente. Para ello, haremos uso de diagramas de caja,  histogramas, diagramas de dispersión y matrices de correlación. 
## Instalar la libreria para la visualizacion
En primer lugar, se deben instalar e importar las librerías necesarias para poder llevar a cabo las visualizaciones.
```{r message= FALSE, warning=FALSE}
library(gridExtra)
library(GGally)
library(dplyr)
...
La matriz de correlaciones muestra los valores de la correlación Pearson. Este coeficiente mide el grado de relación lineal (fuerza y dirección) entre cada par de elementos, tomando valores entre -1 (fuerte correlación negativa) y 1 (fuerte correlación positiva). 
Como se muestra a continuación los coeficientes de correlación entre las calificaciones y el precio (-0.021), el número de reviews (0.049) y el tamaño (0.066) son todos cercanos a 0, y por tanto, se puede afirmar que no existe una relación lineal entre dichas variables y la calificación de la app.
```{r message= FALSE, warning=FALSE}
...
## Matriz de correlaciones
La siguiente gráfica muestra los coeficientes de correlación Pearson a modo de heat map. Se puede observar que existe una correlación lineal moderada entre el número de reviews y el número de instalaciones, con un coeficiente cercano al 0.6. Sin embargo, como se ha comprobado anteriormente mediante la regresión lineal, el coeficiente de correlación se sitúa en 0.95 (una relación lineal mucho más marcada) al hacer la transformación logarítmica de ambas variables.   
```{r message= FALSE, warning=FALSE}
...
En el gráfico se puede comprobar que existen categorías que poseen una distribución similar, por ejemplo, food and drink y finance. En este caso, en la prueba estadística de Mann-Withney se ha obtenido un p-valor de 1 y por lo tanto no se ha podido rechazar la hipótesis nula. Por el contrario, existen categorías que presentan menor similitud, por ejemplo, education y dating, en este caso se ha obtenido en la prueba de Mann-Whitney un p-valor menor de 0.05 y esta vez sí se ha podido rechazar la hipótesis nula.
...
Los siguientes histogramas permiten, al igual, que la gráfica anterior evaluar la similitud de las diferentes categorías, por ejemplo, se puede comprobar que las categorias food and drink y finance presentan una distribución más similar que dating y education. Tanto esta gráfica como la anterior nos pueden ayudar a comprender mejor los resultados obtenidos en la prueba de Mann-Whitney.
...
+ Se ha determinado, mediante el test de Fligner-Killeen, que las variables *Category* y *Multi_genre* no tienen varianzas homogenias entre sus categorías. Por el contrario, la variable *Type*, muestra varianzas homogénias entre sus grupos ("Paid" y "Free").
...
+ Las diferentes categorías de aplicaciones muestran, mediante el test Kruskal-Wallis, diferencias significativas en sus *mean ranks*, siendo las categorías *Dating*, *Health and fitness* y *Personalization* las que más diferencias significativas mostraban tras analizarlas a pares con las demás categorías. Las medias varían entre 4.37 (*ART_AND_DESIGN*) y 4.04 (*DATING*).

+ Un modelo de random forest consigue explicar el 8% de la varianza de *Rating*, lo que se considera como mal modelo. Se han probado alternativas como regresiones polinómicas, regresión lasso y SVM, sin ninguna mejora significativa. Para una mayor precisión, se deberían incluir más variables en el conjunto de datos que se correlacionen de forma más directa con *Rating*.

+ Se observa una relación lineal entre el número de *installs* y el número de *reviews* con un coeficiente de determinación de 0.959.
